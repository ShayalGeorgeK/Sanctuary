...
const Product = () => {
  ...
  return productData? (
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
          ...
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {
              productData.image.map((item,index)=>(
                <img onClick={()=>setImage(item)} src={item} key={index} className='sm:mb-3 shrink-0 cursor-pointer'/>
              ))
            }
          </div>
      ...
      <div className='mt-20'>
        <div className='flex'>
          <b className='border border-gray-200 px-5 py-3 text-sm gap-1'>Description</b>
        ...
        <div className='flex flex-col gap-4 border border-gray-200 px-6 py-6 text-sm text-gray-500'>
          <p>popularity due to their convenience, accessibility, and the global reach they offer.</p>
          <p>popularity due to their convenience, accessibility, and the global reach they offer</p>
        </div>
      </div>
